...
import type { CustomItem } from '../../services/api';
import { customItemsAPI } from '../../services/api';
...
export default function CustomItemsPage() {
  ...
  const fetchItems = async () => {
    if (!token) return;
    try {
      const response = await customItemsAPI.getAll(token);
      setItems(response.data);
    } catch (err) {
      toast.error('Lỗi khi tải danh sách custom items');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await customItemsAPI.update(token, editingId, formData);
        toast.success('Cập nhật custom item thành công!');
      } else {
        await customItemsAPI.create(token, formData);
        toast.success('Tạo custom item thành công!');
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await customItemsAPI.delete(token, id);
      toast.success('Xóa custom item thành công!');
      fetchItems();
    } catch (err) {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Quản lý Custom Items</h1>
        <button
          onClick={() => {
            setFormData({
              name: '',
      ...
      {isFormOpen && (
        <div className="bg-white p-6 rounded-lg shadow-md mb-6">
          <h2 className="text-xl font-semibold mb-4">
            {editingId ? 'Chỉnh sửa Custom Item' : 'Tạo Custom Item mới'}
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item) => (
            ...
            >
              ...
              <p className="text-xs text-gray-500 mb-3">User: {item.userId}</p>
